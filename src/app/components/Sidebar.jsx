...
export default function Sidebar({ activeTab, setActiveTab, user, onLogout }) {
  ...
  const adminMenu = [
    ...
    { id: 'scholars', label: 'Scholar Applications', icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const menuItems = isAdmin ? adminMenu : (isScholar ? scholarMenu : officerMenu);

  return (
    <aside className="w-64 h-screen fixed left-0 top-0 glass-panel flex flex-col justify-between text-white p-6 z-40 border-r border-gold/20">
      ...
      <div className="flex flex-col gap-4 border-t border-white/10 pt-6">
        ...
        <button
          onClick={onLogout}
          className="flex items-center justify-center gap-3 w-full py-2.5 rounded-lg border border-red-500/30 text-red-400 hover:bg-red-500/10 hover:text-red-300 text-xs font-semibold tracking-wider uppercase transition-all"
        >
